...
def letterbox_image_cv2(image, size):
    iw, ih = image.shape[:2]
    w, h = size
    scale = min(w / iw, h/ih)
    nw = int(iw * scale)
...
class BBoxUtility:
    ...
    def decode_boxes(self, mbox_loc, mbox_priorbox, variances):
        ...
        # prior width and height
        prior_width = mbox_priorbox[:, 2] - mbox_priorbox[:, 0]
        prior_height = mbox_priorbox[:, 3] - mbox_priorbox[:, 1]
        # prior center
        prior_center_x = (mbox_priorbox[:, 2] + mbox_priorbox[:, 0]) / 2
        prior_center_y = (mbox_priorbox[:, 1] + mbox_priorbox[:, 3]) / 2
            ...
            for c in range(self.num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    idx = tf.image.non_max_suppression(tf.cast(boxes_to_process, tf.float32), tf.cast(confs_to_process, tf.float32),
                                                        self.top_k, iou_threshold=self.nms_thresh).numpy()
